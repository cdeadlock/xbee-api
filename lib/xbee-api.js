...
XBeeAPI.prototype.buildFrame = function(frame) {
    if (this.options.module === C.MODULE_ID.WIFI){
        return this.buildFrameWifi(frame);
    }
    else if (this.options.module === C.MODULE_ID.SERIAL_ANY) {
        return this.buildFrameSerial(frame);
    }
    throw new Error("Unknown module:".concat(util.inspect(this.options)));
};

XBeeAPI.prototype.buildFrameWifi = function(frame) {
    var S = this.buildState;
    var offset = 0;
    var length = frame_builder[C.MODULE_ID.WIFI][frame.type].apply(this, [ offset, frame ]);
    return new Buffer(S.buffer.slice(0, length));
};

XBeeAPI.prototype.buildFrameSerial = function(frame) {
  var S = this.buildState;
  var offset = 0;
  ...
  // Write payload
  offset = frame_builder[C.MODULE_ID.SERIAL_ANY][frame.type].apply(this, [ offset, frame ]);
...
XBeeAPI.prototype.parseFrame = function(buffer) {
  ...
  if (this.options.module === C.MODULE_ID.SERIAL_ANY) {
      var frame = {
          type: buffer.readUInt8(3), // Read Frame Type
          module: this.options.module
      };
      // Frame Type Specific Parsing, drop start, legth, type and checksum
      frame_parser[frame.module][frame.type](frame, buffer.slice(4, buffer.length - 1));
      return frame;
  }
    else if (this.options.module === C.MODULE_ID.WIFI){
      var frame = {
          type: buffer.readUInt8(0), // Read Frame Type
          module: this.options.module
      };
      //drop command id and options
      frame_parser[frame.module][frame.type](frame, buffer.slice(2, buffer.length));
      return frame;
  }
    else{
      //should never get here this is caught sooner
      throw new Error("Unknown module:".concat(util.inspect(this.options)));
  }
};
...
XBeeAPI.prototype.rawParser = function() {
  ...
  return function(emitter, buffer) {
    if (self.options.module === C.MODULE_ID.SERIAL_ANY) {
        self.parseRaw(buffer);
    }
      else if (self.options.module === C.MODULE_ID.WIFI) {
        self.parseRawWifi(buffer);
    }
      else {
        throw new Error("Unknown module:".concat(util.inspect(self.options)));
    }
  }
};

XBeeAPI.prototype.parseRawWifi = function(buffer) {
    var S = this.parseState;
    buffer.copy(S.buffer);
    S.offset = buffer.length;
    this.commonEmit(S);
};

XBeeAPI.prototype.commonEmit = function(S) {
  ...
  for(var i = 0; i < buffer.length; i++) {
    ...
    if (S.length > 0 && S.offset == S.length + 4) {
      ...
      if (S.checksum != (255 - (S.total % 256))) {
          this.emit("error", new Error("Checksum Mismatch", S));
      }
      else{
          this.commonEmit(S);
      }
