...
var wifi = frame_parser[C.MODULE_ID.WIFI] = {};
var serial = frame_parser[C.MODULE_ID.SERIAL_ANY] = {};
...
serial[C.FRAME_TYPE.NODE_IDENTIFICATION] = function(frame, buffer) {
...
serial[C.FRAME_TYPE.ZIGBEE_RECEIVE_PACKET] = function(frame, buffer) {
...
serial[C.FRAME_TYPE.ZIGBEE_EXPLICIT_RX] = function(frame, buffer) {
  serial[C.FRAME_TYPE.ZIGBEE_RECEIVE_PACKET](frame, buffer);
};

serial[C.FRAME_TYPE.XBEE_SENSOR_READ] = function(frame, buffer) {
  frame.remote64 = frame_parser.parseAddress(buffer, 0, 8);
  frame.remote16 = frame_parser.parseAddress(buffer, 8, 2);
  frame.receiveOptions = buffer.readUInt8(10);
...
serial[C.FRAME_TYPE.MODEM_STATUS] = function(frame, buffer) {
  frame.modemStatus = buffer.readUInt8(0);
};

//Xbee Wifi IO sample
wifi[C.WIFI_COMMAND_ID.IO_SAMPLE] = function(frame, buffer) {
    frame_parser.ParseIOSamplePayload(frame, buffer.slice(0));
};

serial[C.FRAME_TYPE.ZIGBEE_IO_DATA_SAMPLE_RX] = function(frame, buffer) {
  frame.remote64 = frame_parser.parseAddress(buffer, 0, 8);
  frame.remote16 = frame_parser.parseAddress(buffer, 8, 2);
  frame.receiveOptions = buffer.readUInt8(10);
...
//Xbee Wifi AT command response
wifi[C.WIFI_COMMAND_ID.REMOTE_RESPONSE] = function(frame, buffer) {
...
serial[C.FRAME_TYPE.AT_COMMAND_RESPONSE] = function(frame, buffer) {
  frame.id = buffer.readUInt8(0);
  frame.command = String.fromCharCode(buffer.readUInt8(1), buffer.readUInt8(2));
  frame.commandStatus = buffer.readUInt8(3);
  frame.commandData = toArray(buffer.slice(4));
...
serial[C.FRAME_TYPE.REMOTE_COMMAND_RESPONSE] = function(frame, buffer) {
  frame.id = buffer.readUInt8(0);
...
serial[C.FRAME_TYPE.ZIGBEE_TRANSMIT_STATUS] = function(frame, buffer) {
  frame.id = buffer.readUInt8(0);
...
frame_parser.ParseIOSamplePayload = function(frame, buffer) {
  ...
  if (mskD > 0) {
    ...
    for (var bit in C.DIGITAL_CHANNELS.MASK[frame.module]) {
      if ((mskD & (1 << bit)) >> bit) {
        frame.digitalSamples[C.DIGITAL_CHANNELS.MASK[frame.module][bit][0]] = (valD & (1 << bit)) >> bit;
      }
    }
  }
  ...
  if (mskA > 0) {
    ...
    var sampleNr = 0;
    for (var bit in C.ANALOG_CHANNELS.MASK[frame.module]) {
      if ((mskA & (1 << bit)) >> bit) {
        var valA = (analogSamples[sampleNr*2] << 8) | analogSamples[sampleNr*2+1];
        // Convert to mV, resolition is < 1mV, so rounding is OK
        frame.analogSamples[C.ANALOG_CHANNELS.MASK[frame.module][bit][0]] = Math.round((valA * 1200) / 1023);
        sampleNr++;
      }
    }
  }
...
// Series 1 Support
serial[C.FRAME_TYPE.TX_STATUS] = function(frame, buffer) {
  frame.id = buffer.readUInt8(0);
  frame.deliveryStatus = buffer.readUInt8(1);
};

serial[C.FRAME_TYPE.RX_PACKET_64] = function(frame, buffer) {
  frame.remote64 = frame_parser.parseAddress(buffer, 0, 8);
...
serial[C.FRAME_TYPE.RX_PACKET_16] = function(frame, buffer) {
  frame.remote16 = frame_parser.parseAddress(buffer, 0, 2);
...
serial[C.FRAME_TYPE.RX_PACKET_64_IO] = function(frame, buffer) {
  frame.remote64 = frame_parser.parseAddress(buffer, 0, 8);
  frame.rssi = buffer.readUInt8(8);
  frame.receiveOptions = buffer.readUInt8(9);
  frame.data = toArray(buffer.slice(10));
...
serial[C.FRAME_TYPE.RX_PACKET_16_IO] = function(frame, buffer) {
  frame.remote16 = frame_parser.parseAddress(buffer, 0, 2);
  frame.rssi = buffer.readUInt8(2);
  frame.receiveOptions = buffer.readUInt8(3);
  frame.data = toArray(buffer.slice(4));
